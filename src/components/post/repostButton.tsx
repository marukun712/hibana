...
import { deleteEvent, isReposted, postEvent } from "~/lib/api/event";
import { getCurrentUser } from "~/lib/api/users";
import { AiOutlineRetweet, AiOutlineEdit } from "solid-icons/ai";
...
export default function RepostButton(props: { 
  ...
  async function repost() {
    const eventId = await postEvent("event.repost", { target: props.originalPost.id });
    if (eventId) {
      setReposted(true);
      setRepostId(eventId);
    }
  ...
  onMount(async () => {
    ...
    const result = await isReposted(publickey, props.originalPost.id);
