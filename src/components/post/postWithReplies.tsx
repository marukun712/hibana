...
import type { PostWithReplies } from "~/utils/replyUtils";
		...
		if (post.event === "event.repost" && post.target) {
			return (
				<RepostedPost
					originalPost={{
						id: post.target.id,
						text: (post.target.message as EventContent & { content: string })
							.content,
				...
				depth < maxDepth && (
					<div class="ml-4">
						<For each={props.post.replies}>
							{(reply) => (
								<PostWithRepliesComponent post={reply} depth={depth + 1} />
							)}
						</For>
					</div>
				)}
			{props.post.replies &&
				...
				depth >= maxDepth && (
					<div class="ml-4 p-2 text-sm text-base-content/60 border-l border-base-300 pl-4">
						<span>
							さらに {props.post.replies.length} 件のリプライがあります
						</span>
					</div>
				)}
		</div>
